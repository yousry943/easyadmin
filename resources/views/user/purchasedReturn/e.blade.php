...
@include('user.includes.header')
    ...
    <section class="site-data-container accountingForm">
        <form action="{{url('/addReturnPurchase')}}" method="post">
            {{csrf_field()}}
                        ...
                        <div class="billInputs flexBetween">
                            ...
                            <div class="billInputsRight">
                                ...
                                <div class="flexBetween billOpject"><label for="">item</label><input
                                       class="widthInputQ" type="text" value="{{$recive_item->items[0]->name}}" disabled></div></div>
                                <div class="billInputsRight">
                                    <div class="flexBetween billOpject">
                                <label for="">quantity</label>
                           <input class="widthInputQ" type="number" max="{{$recive_item->quantity}}" value="{{$recive_item->quantity}}" name="quanty[]" ></div></div>
                                    <input type="hidden" name="item[]" value="{{$recive_item->item_id}}" ></div>
                                <input type="hidden" name="price[]" value="{{$recive_item->price}}" ></div>
